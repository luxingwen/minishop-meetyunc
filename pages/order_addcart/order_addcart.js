...
Page({
    ...
    subCollect: function(event) {
        ...
        if (selectValue.length > 0) {
            ...
            request.collectProductAll({
                data: {
                    productIds: selectValueProductId,
                },
                success: res => {
                    wx.showToast({
                        title: res.msg,
    ...
    checkboxAllChange: function(event) {
        ...
        if (value.length > 0) { this.setAllSelectValue(1) } else { this.setAllSelectValue(0) }
    },
    ...
    switchSelect: function() {
        ...
        if (selectValue.length < 1) { that.setData({ selectCountPrice: selectCountPrice }); } else {
    ...
    setCartNum(cartId, cartNum, successCallback) {
        var that = this;
        request.changeCartNum({
          data: { cartId: cartId, cartNum: cartNum },
          success: res => {
    ...
    getCartList: function() {
        ...
        request.cartList({
          success: res=> {
            ...
            if (valid.length > 0) {
                for (var index in valid) {
                    if (valid[index].cart_num == 1) { valid[index].numSub = true; } else { valid[index].numSub = false; }
